...
import { CircularProgress, FormLabel, Grid, MenuItem } from "@mui/material";
...
import { selectCategories, selectModelCategories, selectModelCategories_status } from '../../../../data/categories';
import { useRouter, useSearchParams } from 'next/navigation';
import { selectAllColors, selectAllColors_status } from '../../../../data/get_all_colors';
import { selectAllMaterials, selectAllMaterials_status } from '../../../../data/get_all_materials';
import { selectAllBrands, selectAllBrands_status } from '../../../../data/get_all_brands';
import { selectModelPlatforms, selectModelPlatforms_status } from '../../../../data/get_model_platforms';
import { selectRenderPlatforms, selectRenderPlatforms_status } from '../../../../data/get_render_platforms';
...
const supportedFileTypes = 'application/x-compressed, application/x-zip-compressed, application/x-rar-compressed, application/x-7z-compressed, application/zip, application/rar, application/7z, .zip, .rar, .7z'
...
export function AddModelForm({ editing = false, model, selectedBrand }: { editing?: boolean, model?: any, selectedBrand?: any }) {
  ...
  const stylesData_status = useSelector(selectAllStyles_status)
  const categoriesData_status = useSelector(selectModelCategories_status);
  const colorsData_status = useSelector(selectAllColors_status);
  const materialsData_status = useSelector(selectAllMaterials_status);
  const brandsData_status = useSelector(selectAllBrands_status);
  const modelPlatformsData_status = useSelector(selectModelPlatforms_status);
  const renderPlatformsData_status = useSelector(selectRenderPlatforms_status);
  ...
  return (
    ...
    >
      <Box sx={{ width: '100%', minHeight: '70dvh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {
          stylesData_status == 'succeeded' &&
            categoriesData_status == 'succeeded' &&
            colorsData_status == 'succeeded' &&
            materialsData_status == 'succeeded' &&
            brandsData_status == 'succeeded' &&
            modelPlatformsData_status == 'succeeded' &&
            renderPlatformsData_status == 'succeeded'
            ?
            <Formik
              ...
              initialValues={{
                ...
                model_platform_id: editing && model?.model_platform_id ? model?.model_platform_id : modelPlatformsData?.platforms?.find(e => String(e.name).toLowerCase() == '3ds max')?.id || '',
                render_platform_id: editing && model?.render_platform_id ? model?.render_platform_id : renderPlatformsData?.platforms?.find(e => String(e.name).toLowerCase() == '3ds max')?.id || '',
                colors: [],
                materials: [],
                file: '',
                cover: '',
