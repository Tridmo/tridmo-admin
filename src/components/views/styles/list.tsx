...
export default function StylesList() {
  ...
  const all__styles_status = useSelector(selectAllStyles_status)
  const styles = useSelector(selectAllStyles)

  const [selectedStyle, setSelectedStyle] = useState<any>(null)

  function handleClick(event: any, style: any) {
  ...
  function handleOpenAddModal() {
    dispatch(setStyleFormState({ open: true, editing: false }))
    dispatch(setOpenModal(true))
    handleClose()
  }
  function handleOpenEditModal() {
    dispatch(setStyleFormState({ open: true, editing: true, style: selectedStyle }))
    dispatch(setOpenModal(true))
    handleClose()
  }
  ...
  function handleClickDelete() {
    const modalContent: ConfirmContextProps = {
      message: `Вы уверены, что хотите удалить стиль «${selectedStyle?.name}»?`,
      actions: {
        on_click: {
          args: [selectedStyle?.id],
          func: async (checked: boolean, id: number) => {
            dispatch(setConfirmProps({ is_loading: true }))
            instance.delete(`styles/${id}`)
              .then(res => {
                if (res?.data?.success) {
  ...
  return (
    <>
      ...
      >
        ...
        {
          all__styles_status == 'succeeded' ?
            (
              styles && styles?.data?.length != 0
                ? styles?.data?.map((style, index: any) =>
